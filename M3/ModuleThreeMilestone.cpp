...
void URender()
{
    ...
    // 1. Scales the object by 25 on the x-axis, 15 on the y axis.
    glm::mat4 scale = glm::scale(glm::vec3(25.0f, 15.0f, 0.0f));
    ...
    // translate the camera backward by fifteen units (z-axis)
    glm::mat4 view = glm::translate(glm::vec3(0.0f, 0.0f, -15.0f));
    ...
    glm::mat4 projection = glm::perspective(glm::radians(125.0f), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
    ...
    // 2. Rotates shape by 90 degrees on the x and z axis
    rotation = glm::rotate(90.0f, glm::vec3(1.0f, 0.0f, 1.0f));

    // Rotate the shape on its y-axis -16 degrees
    rotation = glm::rotate(rotation, glm::radians(-16.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    // translate the camera backward by five units (z-axis)
    ...
    // Third parameter is the distance of the near plane to the camera
    // Fourth parameter is the distance of the far plane to the camera
    projection = glm::perspective(glm::radians(125.0f), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
